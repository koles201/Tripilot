...
The Business Profile feature enables users (business owners) to create a profile and submit verification documents. Admins review and approve or reject submissions.
...
Creates a new business profile for the authenticated user (only one allowed per user).
...
```json
{
  ...
  "postalCode": "11000",
  "phone": "+420123456789",
  "email": "contact@cafearora.example",
  "website": "https://cafearora.example",
  "logoUrl": "https://cdn.example.com/logos/aurora.png",
  "bannerUrl": "https://cdn.example.com/banners/aurora-hero.jpg"
}
```

### PUT /api/business-profiles/{id}
Partial update of owned business profile. Null or omitted properties are ignored.

### POST /api/business-profiles/{id}/submit-verification
Submits the business profile for verification. Requires at least one document URL.

Request Body:
```json
{
  "documentUrls": ["https://storage.example.com/docs/license.pdf"]
}
```

### POST /api/business-profiles/{id}/approve (Admin only)
Approves a submitted business profile.

### POST /api/business-profiles/{id}/reject (Admin only)
Rejects a submitted business profile, providing a reason.

Request Body:
```json
{
  "rejectionReason": "Incomplete registration certificate"
}
```

### GET /api/business-profiles (Admin only)
Paginated list of business profiles with optional filtering.
  ...
  "items": [
    {
      ...
      "verificationSubmittedAt": "2025-10-30T12:10:00Z"
    }
  ],
  "total": 1,
  "page": 1,
...
States lifecycle: `Pending -> Submitted -> (Approved | Rejected)`; after rejection owner may submit again.

Notifications: Submission, approval, and rejection trigger stub email logs (to be replaced with real provider).
