...
public class GetReviewsForPlaceQueryHandler : IRequestHandler<GetReviewsForPlaceQuery, List<ReviewResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public GetReviewsForPlaceQueryHandler(IUnitOfWork unitOfWork, IMapper mapper)
    {
        _unitOfWork = unitOfWork;
        _mapper = mapper;
    }

    public async Task<List<ReviewResponse>> Handle(GetReviewsForPlaceQuery request, CancellationToken cancellationToken)
    {
        var query = _unitOfWork.Repository<Review>()
            .GetQueryable()
            .Include(r => r.Reviewer)
            .Include(r => r.Place)
            .Where(r => r.PlaceId == request.PlaceId && r.IsActive);
